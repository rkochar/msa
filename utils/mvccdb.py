...
def create_mvccdb():
    ...
    sql_init_lambda = m.create_lambda('sqldb-init', "mvccdb/init", "mvcc_sqldb_init.sqldb_init", template="http_sql", environment=sqldb_lambda_environment, role=sql_lambda_iam_role, imports=["pymysql"], is_timed=True, is_ram=True, is_telemetry=True, opts=ResourceOptions(depends_on=[sqldb]))
    sql_get_lambda = m.create_lambda('sqldb-get', "mvccdb/get", "mvcc_sqldb_get.sqldb_get", template="http_sql", environment=sqldb_lambda_environment, role=sql_lambda_iam_role, imports=["pymysql", "pydantic"], is_timed=True, is_ram=True, is_telemetry=True, opts=ResourceOptions(depends_on=[sqldb]))
    ...
    worker_lambda = m.create_lambda('worker', "mvccdb/worker", "worker.check_transaction", template="http_sql_pub",  environment=worker_environment | sqldb_lambda_environment, role=mq_sql_lambda_iam_role, imports=["pymysql", "pydantic"], is_timed=True, is_ram=True, is_telemetry=True, opts=ResourceOptions(depends_on=[sqldb, transaction_mq]))
    control_lambda = m.create_lambda('control', "mvccdb/control", "control.confirm_transaction", template="mq_sql", role=mq_sql_lambda_iam_role, mq_topic=transaction_mq, environment=worker_environment | sqldb_lambda_environment, imports=["pymysql"], is_timed=True, is_ram=True, is_telemetry=True, opts=ResourceOptions(depends_on=[transaction_mq, sqldb, worker_lambda]))
