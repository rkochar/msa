...
def create_mvccdb():
    ...
    sql_init_lambda = m.create_lambda('sqldb-init', "mvccdb/init", "mvcc_sqldb_init.sqldb_init", template="http_sql", environment=sqldb_lambda_environment, role=sql_lambda_iam_role, imports=["pymysql"], opts=ResourceOptions(depends_on=[sqldb]))
